"""
Flask API for requesting data from dot_data script.
"""
...
def home():
    """
    Function for root-level API Help Document
    """
...
<p><b>EXAMPLE:</b> For a single point, you would request data like this: 
<a href='http://{gethostname()}/api/percentiles?xcoord=-2173040&ycoord=882215'>
http://{gethostname()}/api/percentiles?xcoord=-2173040&ycoord=882215</a>"""


@app.route("/api/percentiles", methods=["GET"])
def api_request_percentiles():
    """
    Function for API call to backend script for dot_data get_percentile_data()

    :return: Pickle formatted xarray.DataArray object
    """
    if "xcoord" in request.args:
        xcoord = request.args["xcoord"]
    else:
        return "Error: No X coordinate provided. Please specify an X coordinate ."
